...
class DeliveryCarrier(models.Model):
    ...
    def purchase_rate_shipment(self, order):
        ...
        if not self or self.destination_type == "one":
            return super().purchase_rate_shipment(order)
        self.ensure_one()
        partner = self._get_delivery_partner_from_purchase(order)
        ...
        # Return the same structure as the standard rate methods instead of
        # raising, as this is called from a stored compute method and an
        # exception would block any change done in the order.
        return {
            "success": False,
            "price": 0.0,
            "error_message": self.env._("There is no matching delivery rule."),
        ...
        for p in pickings:
            ...
            for subcarrier in carrier.child_ids.filtered(
                ...
                if subcarrier.delivery_type == "fixed":
                    ...
                    try:
                        # on purchase_base_on_rule_send_shipping, the method
                        # _purchase_get_price_available is called using
                        # p.carrier_id, ignoring the self arg, so we need to
                        # temporarily replace it with the subcarrier
