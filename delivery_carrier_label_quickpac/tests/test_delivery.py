...
class TestDelivery(common.SavepointCase):
        ...
        cls.carrier = cls.env['delivery.carrier'].create(
            {
                ...
                'product_id': cls.env.ref(
                    'delivery_carrier_label_quickpac.product_quickpac_service'
                ).id,
            }
        )
    ...
    def test_carrier_option_allowed_tmpl_options_ids_1_with_context(self):
        ...
        tmpl_options = self.env.ref(
            'delivery_carrier_label_quickpac.quickpac_layout_option_a6'
        )
        carrier_option = self.carrier_option.with_context(
            default_allowed_tmpl_options_ids=tmpl_options.ids
        )
        self.assertEqual(carrier_option.allowed_tmpl_options_ids, tmpl_options)
    ...
    def test_carrier_quickpac_basic_service_ids(self):
        self.assertFalse(self.carrier.quickpac_basic_service_ids)
    ...
    def test_carrier_allowed_tmpl_options_ids_2(self):
        ...
        self.carrier.available_option_ids += self.create_carrier_option(
            values={
                'quickpac_type': 'resolution',
                'mandatory': True,
                'carrier_id': self.carrier.id,
            }
        )
        single_option_types = [
            x for x in SINGLE_OPTION_TYPES if x != 'resolution'
        ]
        services = self.env['delivery.carrier.template.option'].search(
            [('quickpac_type', 'in', single_option_types)]
        )
