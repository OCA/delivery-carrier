...
class TestDeliveryCarrier(EasypostTestBaseCase):
    ...
    def test_easypost_oca_order_rate_shipment(self):
        ...
        with patch.object(
            EasypostRequest, "calculate_shipping_rate", return_value=mock_rate
        ):
    ...
    def test_easypost_oca_multiple_packages_shipping(
        ...
        Picking.move_ids[0].move_line_ids.result_package_id.shipping_weight = 5.0
    ...
    def test_easypost_oca_shipping_error_handling(
        ...
        mock_create_shipment.side_effect = UserError(
            f"Error: {mock_error.message}\nError Body: {mock_error.errors[0]['http_body']}"
        )
