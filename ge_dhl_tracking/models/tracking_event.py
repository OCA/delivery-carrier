...
class TrackingEvent(models.Model):
    ...
    # source_transfer_id = fields.Many2one("stock.picking", help="Transfer which created the transport to the customer.")
    ...
    def _compute_message(self):
        for record in self:
            record.ric_name = self.env["dhl.ric"].search(
                [("code", "=", record.ric)], limit=1).name
            record.ice_name = self.env["dhl.ice"].search(
                [("code", "=", record.ice)], limit=1).name
            record.standard_event_name = self.env["dhl.ric"].search(
                [("code", "=", record.standard_event_code)], limit=1).name
